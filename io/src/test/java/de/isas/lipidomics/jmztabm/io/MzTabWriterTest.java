...
//        System.out.println(mtd);
    ...
    public void testWriteSmallMoleculeSummaryToTsvWithJackson() throws IOException {
        ...
        try (StringWriter sw = new StringWriter()) {
            ...
            Assert.assertEquals(2, smallMoleculeSummary.split("\r\n|\r|\n").length);
        }
    }
    ...
    public void testWriteSmallMoleculeFeaturesToTsvWithJackson() throws IOException {
        ...
        try (StringWriter sw = new StringWriter()) {
            ...
            Assert.assertEquals(1, smallMoleculeFeatures.split("\r\n|\r|\n").length);
        }
    }
    ...
    public void testWriteSmallMoleculeEvidenceToTsvWithJackson() throws IOException {
        ...
        try (StringWriter sw = new StringWriter()) {
            ...
            //check for exactly one header line
            Assert.assertEquals(1, smallMoleculeEvidences.split("\r\n|\r|\n").length);
        }
    }
    ...
    public void testLargeNumberOfFeaturesFromMTBLS263() throws URISyntaxException, IOException, MZTabException {
        ...
        sms.forEach((smallMolecule) -> {
            for (int i = 0; i < expFactor; i++) {
                try {
                    SmallMoleculeSummary clone = clone(SmallMoleculeSummary.class, smallMolecule);
                    clone.setSmlId((smsIds.get() + i) + "");
                    newSms.add(clone);
                } catch (IOException ex) {
                    Logger.getLogger(MzTabWriterTest.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        ...
        List<SmallMoleculeFeature> newSmf = new ArrayList<>(smf.size() * expFactor);
        newSmf.addAll(smf);
        smf.forEach((smallMoleculeFeature) -> {
            for (int i = 0; i < expFactor; i++) {
                try {
                    SmallMoleculeFeature clone = clone(SmallMoleculeFeature.class, smallMoleculeFeature);
        ...
        List<SmallMoleculeEvidence> newSme = new ArrayList<>(sme.size() * expFactor);
        newSme.addAll(sme);
        sme.forEach((smallMoleculeEvidence) -> {
            for (int i = 0; i < expFactor; i++) {
                try {
                    SmallMoleculeEvidence clone = clone(SmallMoleculeEvidence.class, smallMoleculeEvidence);
        ...
        File tempFile = File.createTempFile("testLargeNumberOfFeaturesFromMTBLS263",
                ".mztab");
        MzTabWriter writer = new MzTabWriter();
        writer.write(tempFile.toPath(), mzTabFile);
        long lines = Files.lines(tempFile.toPath()).count();
        Assert.assertEquals(
                53 +//metadata lines
                1 + 17+//summary header and original data
                (17 * expFactor)+//cloned summary data 
                1 + 19 + //feature header and original data
                (19 * expFactor)+//cloned feature data 
                1 + 19 + //evidence header and original data
