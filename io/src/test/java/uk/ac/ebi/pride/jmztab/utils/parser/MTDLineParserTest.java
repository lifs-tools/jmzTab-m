...
public class MTDLineParserTest {
    ...
    public void testUnitParser() throws Exception {
        ...
        assertTrue(metadata.getMzTabVersion().equals("1.0 rc5"));
...
//        assertTrue(metadata.getTabDescription().getType() == MZTabDescription.Type.Identification);
        ...
        assertTrue(metadata.getMzTabID().equals("PRIDE_1234"));
        ...
        Parameter param = context.getSampleProcessingMap().get(1).getSampleProcessing().get(0);
        ...
        assertTrue(context.getSampleProcessingMap().size() == 2);
        param = context.getSampleProcessingMap().get(12).getSampleProcessing().get(0);
        assertTrue(param instanceof Parameter);
        cvParam = (Parameter) param;
        assertTrue(cvParam.getName().contains("enzyme digestion"));
        assertTrue(MZTabUtils.isEmpty(cvParam.getValue()));
        param = context.getSampleProcessingMap().get(12).getSampleProcessing().get(1);
        assertTrue(param instanceof Parameter);
        cvParam = (Parameter) param;
        ...
        param = context.getInstrumentMap().get(1).getInstrumentName();
        assertTrue(param.toString().contains("LTQ Orbitrap"));
        List<Parameter> analyzerList = context.getInstrumentMap().get(1).getInstrumentAnalyzer();
        assertTrue(analyzerList.size() == 1);
        param = context.getInstrumentMap().get(2).getInstrumentSource();
        assertTrue(param.toString().contains("ETD"));
        param = context.getInstrumentMap().get(13).getInstrumentDetector();
        ...
        param = context.getSoftwareMap().get(11).getParameter();
        assertTrue(param.toString().contains("Mascot"));
        List<String> settingList = context.getSoftwareMap().get(2).getSetting();
...
//        assertTrue(metadata.getFalseDiscoveryRate().size() == 3);
        ...
        assertTrue(metadata.getPublications().size() == 2);
        ...
        assertTrue(metadata.getContacts().size() == 2);
        ...
        assertTrue(metadata.getUri().size() == 2);
...
//        assertTrue(metadata.getPeptideQuantificationUnit() != null);
        ...
        assertTrue(context.getCvMap().size() == 2);
        assertTrue(context.getCvMap().get(1).getVersion().equals("3.54.0"));
        assertTrue(context.getCvMap().get(12).getUrl().equals("http://psidev.cvs.sourceforge.net/viewvc/psidev/psi/psi-ms/mzML/controlledVocabulary/psi-ms.obo"));
    }
    ...
    public void testMsRun() throws Exception {
        ...
        assertTrue(msRun2.getFragmentationMethod().get(0).getCvAccession().equals("MS:1000133"));
    ...
    public void testSample() throws Exception {
        ...
        Sample sample1 = context.getSampleMap().get(1);
        assertTrue(sample1.getSpecies().size() == 2);

        parser.parse(1, "MTD\tsample[1]-tissue[1]\t[BTO, BTO:0000759, liver, ]", errorList);
        assertTrue(sample1.getTissue().size() == 1);

        parser.parse(1, " MTD\tsample[1]-cell_type[1]\t[CL, CL:0000182, hepatocyte, ]", errorList);
        assertTrue(sample1.getCellType().size() == 1);
        ...
        assertTrue(sample1.getDisease().size() == 2);
        ...
        Sample sample2 = context.getSampleMap().get(2);
        ...
        assertTrue(sample1.getCustom().size() == 2);
    }
...
//        assertTrue(context.getAssayMap().get(1).().getAccession().equals("PRIDE:0000114"));
        ...
        assertTrue(context.getAssayMap().get(1).getSampleRef().equals(sample1));
        assertTrue(context.getAssayMap().get(2).getSampleRef().equals(sample2));
        ...
        assertTrue(context.getAssayMap().get(1).getMsRunRef().equals(msRun1));
        assertTrue(context.getAssayMap().get(2).getMsRunRef().equals(msRun2));
...
//        assertTrue(quantificationMod.getPosition().equals("Anywhere"));
    ...
    public void testStudyVariable() throws Exception {
        ...
        assertTrue(context.getStudyVariableMap().size() == 1);
        assertTrue(context.getStudyVariableMap().get(1).getDescription().equals("Group B (spike-in 0,74 fmol/uL)"));
        ...
        assertTrue(context.getStudyVariableMap().get(1).getSampleRefs().size() == 2);
        assertTrue(context.getStudyVariableMap().get(1).getSampleRefs().get((2-1)) == sample2);
